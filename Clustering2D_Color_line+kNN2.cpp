...
int main(int argc, char *argv[])
{
	...
	int recognize = -1;											// kNNによる認識の有無（最初は認識なしモード）
	...
	while (1) {
		...
		for (int i = 0; i < numberPeopleDetected; i++) {
			...
			for (int j = 0; j < numberBodyParts; j++) {
				...
				drawJoint(i, j, c, x, y, colorImage);
			...
			drawLimb(observedPosture, 0, i, colorImage);
...
int clusteringPostures(Mat posturesForClustering, int numberOfClusters, Mat *labels)
{
	...
	for (int i = 0; i < numberOfClusters; i++) {
		...
		for (int j = 0; j < NUMBER_OF_JOINTS; j++) {
			...
			drawJoint(i, j, c, x, y, centroidImage);
		}

		drawLimb(centers, i, i, centroidImage);
